...
from presto_hive_lib import get_presto_records_raise_exception
...
global_table_column_file_name = pathlib.Path("")
global_table_row_count_file_name = pathlib.Path("")
...
def get_s3_parquet_file(s3_location, parq_file_path, client_kwargs=None):
    ...
    print(
        "Saved to file {}, size {:,d} bytes".format(
...
class HiveDatabase:
    ...
    def _show_create_tables(database):
        ...
        sql = "show tables"
        host, port = get_hive_host_port()
        tables = get_hive_records_database_like_table(host, port, sql, database)
    ...
    def show_tblproperties(self, database, table):
        ...
        formatter = {
            "transient_lastDdlTime": lambda ts: datetime.datetime.fromtimestamp(
                int(ts)
            )
        }
        print_name_value_dict(name_value, formatter)
    ...
    def show_table_location(self, database, table):
        ...
        table_location = get_hive_table_location(
            host, port, table, database_valid
        )
        if table_location:
        ...
        for table_name in metadata.tables:
            ...
            for idx, column in enumerate(tbl.columns):
                if not isinstance(column.type, sa.types.NullType):
                    ...
                    col_type = "UNKNOWN"
                col_info = TableColumnInfo(
                    str(column.name), str(col_type), column.nullable
                )
...
def print_presto_column_distinct_values(
    ...
    if column_type in ["varchar"]:
        ...
        print("Not printing column {} of type {}".format(column, column_type))
    else:
...
class PrestoDatabase:
    ...
    def show_column_distinct(self, catalog, schema, table, column):
        ...
        if column not in columns.Column.values:
            print(
                "Column {} does not exist in {} {} {}".format(
        ...
        for column in columns.Column.values:
            if is_analytics_hourly_group_column(column):
                ...
                if column_type.size == 1:
                    ...
                    msg = "Cannot get column_type for column {}".format(column)
                    assert False, msg
    ...
    def desc_table(self, catalog, schema, table):
        """describe table"""

        sql = "describe {}.{}.{}".format(catalog, schema, table)
        host, port = get_presto_host_port()
        df = get_presto_records(host, port, sql)
        rows, cols = df.shape
        if rows > 0:
    ...
    def _get_table_desc(self, catalog, schema, table):
        """ get column descriptions

            returns None if database error
        """
        ...
        df = None
        try:
            df = get_presto_records_raise_exception(host, port, sql)
        except pyhive.exc.DatabaseError:
            df = None
        if df is not None:
            rows, cols = df.shape
            df['Table'] = [table] * rows
    ...
    def _get_table_row_count(self, catalog, schema, table):
        """ get number of rows in a table

            returns -1 if error
        """

        sql = "select count(*) as row_count from {}.{}.{}".format(
            catalog, schema, table)
        host, port = get_presto_host_port()
        df = None
        try:
    ...
    def _save_table_row_count(self, table_list, row_count_list, file_name):
        table_row_count = pd.DataFrame({
            'Table': table_list,
            'row_count': row_count_list
        })
        table_row_count_file = pathlib.Path(file_name)
        if table_row_count_file.exists():
            table_row_count_file.unlink()
        table_row_count.to_csv(
            table_row_count_file, sep='\t', index=False)

    def _save_tables_row_count(self, catalog, schema, table_list, file_name):
        row_count_list = []
        for idx, table in enumerate(table_list):
            print(f'row count for {table} {idx}/{len(table_list)}')
            table_row_count = self._get_table_row_count(
                catalog, schema, table)
            row_count_list.append(table_row_count)
            self._save_table_row_count(
                table_list[:idx + 1], row_count_list, file_name)

    def desc_tables(self, catalog, schema):
        """describe all tables from catalog and schema"""
        ...
        column_count = 0
        for idx, table in enumerate(table_list):
            print(f'column info for {table} {idx}/{len(table_list)}')
            df = self._get_table_desc(catalog, schema, table)
            if df is not None:
        ...
        if not global_table_column_file_name.exists():
            cols = ['index', 'Table', 'Column', 'Type', 'Extra', 'Comment']
            all_tables.reset_index()[cols].to_csv(
                global_table_column_file_name, sep='\t', index=False)
        print(
            "max comment length {}".format(all_tables.Comment.str.len().max())
        )
        ...
        def get_clickhouse_sa_columns(metadata):
            ...
            for idx, col in enumerate(columns):
                ...
                if idx == 0:
                    ...
                    tbl_col = sa_schema.Column(
                        name, ch_types.Nullable(ch_type)
                    )
