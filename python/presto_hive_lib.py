...
def get_presto_records_raise_exception(host, port, sql):
    "runs a presto sql statement and returns result"
    conn = get_presto_connection(host, port)
    df = pd.read_sql(sql, conn)
    return df
...
def get_hive_records_database_like_table(
    host, port, sql, database=None, table=None
):
...
def get_hive_records_database_dot_table(
    ...
    if database is not None and table is not None:
        sql_suffix = " {}.{}".format(database, table)
...
def timed():
    ...
    print(
        "===== End   {:%H:%M:%S} (total: {:.2f} seconds)".format(
...
def get_sa_table_int_min_max(table) -> Optional[Dict]:
    ...
    for idx, column in enumerate(table.columns):
        query = sa.select(
            [sa.literal(column.name), sa.func.min(column), sa.func.max(column)]
        )
...
def get_sa_new_table(
    metadata, table_name, new_table_name, smallest_int_types=False
):
